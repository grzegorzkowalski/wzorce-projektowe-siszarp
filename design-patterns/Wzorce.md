...
1. Ustaw wartoœci domyœlne Total = 100m, Paid = false.
...
Cel: Dodaj now¹ metodê do klasy ReservationMother, która obs³u¿y nowy, konkretny scenariusz.

Scenariusz: "Rezerwacja Last Minute"

Imiê/Nazwisko: "Tomasz Szybki"

Data zameldowania: Dziœ (DateTime.Now)
...
P³atnoœæ: Prawda (HasPaid = true)

Twoje zadanie:
Napisz kod metody CreateLastMinuteReservation() i umieœæ j¹ w klasie ReservationMother.

### Zadanie 2

Scenariusz Zadania: Testowanie Serwisu Faktur
Kontekst: Mamy system ksiêgowy, w którym faktury musz¹ przechodziæ przez ró¿ne stany weryfikacji i p³atnoœci.

1. Model Danych (Klasy Produkcyjne)
Za³ó¿my, ¿e mamy nastêpuj¹c¹ klasê `Invoice`:
```
...
{
    // Metoda do testowania: sprawdza, czy faktura mo¿e byæ zaksiêgowana w systemie
    public bool CanBeBooked(Invoice invoice)
    {
        // Fakturê mo¿na zaksiêgowaæ, jeœli:
...
2. Twoim zadaniem jest stworzenie statycznej klasy `InvoiceMother i zaimplementowanie w niej metod, które bêd¹ zwracaæ faktury skonfigurowane dla konkretnych scenariuszy testowych.

Metody do Utworzenia w `InvoiceMother`:
Nazwa Metody	Opis Scenariusza	Konfiguracja Danych
`CreateValidPLN()`	Faktura gotowa do zaksiêgowania	IsApproved = true, IsPaid = false, Currency = "PLN", Amount = 1000.00M, Issue/Due Date – dzisiaj/za 30 dni.
`CreateUnapproved()`	Faktura oczekuj¹ca na kontrolê	IsApproved = false, IsPaid = false, Currency = "PLN", Amount = 500.00M.
...
3. Napisz Testy
Po zaimplementowaniu `InvoiceMother`, u¿yj jej, aby napisaæ dwa testy xUnit dla metody `InvoiceService.CanBeBooked()`:

Test 1 (Fact): SprawdŸ, czy `CanBeBooked()` zwraca true dla faktury stworzonej przez `InvoiceMother.CreateValidPLN()`.

Test 2 (Fact): SprawdŸ, czy `CanBeBooked()` zwraca false dla faktury stworzonej przez `InvoiceMother.CreateUnapproved()`.
...
1. Napisz testy z wykorzystanie wzorca Page Object, który na stronie https://kidlab.pl, odnajdzie link O Nas i sprawdzi czy modal siê otworzy.
1. Na modalu znajdz tekst "Nasza oferta zajêæ".
